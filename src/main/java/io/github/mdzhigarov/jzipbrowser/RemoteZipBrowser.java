 ...
 */
 ...
 public class RemoteZipBrowser implements ZipBrowser {
     ...
     private static final int LOCAL_FILE_HEADER_SIGNATURE = 0x04034b50; // "PK\003\004"
     ...
     // --- ZIP File Format Constants and Sizes ---
     ...
     private static final long ZIP64_MARKER = 0xFFFFFFFFL; // Indicates ZIP64 values are in extra field
     private static final int MIN_LOCAL_FILE_HEADER_SIZE = 30;
     private static final int EOCD_RECORD_SIZE = 22;
     private static final int ZIP64_EOCD_LOCATOR_SIZE = 20;
     private static final int ZIP64_EOCD_RECORD_SIZE = 56;
     private static final int MIN_CENTRAL_DIRECTORY_ENTRY_SIZE = 46;
     ...
     private RemoteZipBrowser(URL zipUrl, HttpClient httpClient, String basicAuth, 
         ...
         this.entries = Collections.unmodifiableMap(entries); // Make it read-only
    }
     ...
     private CompletableFuture<byte[]> fetchFileData(ZipEntry entry) {
         ...
         long headerEnd = headerStart + MIN_LOCAL_FILE_HEADER_SIZE;
             ...
             .thenCompose(headerData -> {
                 try {
                     ...
                     if (isZip64OrStreamingFormat(localCompressedSize, localUncompressedSize)) {
                         ...
                         logger.debug("Using central dir compressed size: {}", actualCompressedSize);
                     } else {
                         actualCompressedSize = localCompressedSize;
                     }
 
                     // Calculate actual data offset using local header values
                     long dataOffset = headerStart + MIN_LOCAL_FILE_HEADER_SIZE + localFileNameLength + localExtraFieldLength;
     ...
     public static class Builder implements ZipBrowser.Builder {
         ...
         public CompletableFuture<ZipBrowser> build() {
             ...
             return getFileSize(client, basicAuth)
                 .thenCompose(size -> 
                     parseCentralDirectory(client, basicAuth, size)
                         .thenApply(entries -> new RemoteZipBrowser(url, client, basicAuth, size, entries))
                 );
         }
                 ...
                 .thenCompose(data -> {
                     ...
                     for (int i = data.length - 4; i >= 0; i--) {
                         ...
                         if (signature == EOCD_SIGNATURE) {
                             ...
                             if (i + EOCD_RECORD_SIZE > data.length) {
                             ...
                             ByteBuffer eocdBuffer = ByteBuffer.wrap(data, i, EOCD_RECORD_SIZE);
                             eocdBuffer.order(java.nio.ByteOrder.LITTLE_ENDIAN); // ZIP files use little-endian byte order
                             eocdBuffer.getInt(); // Skip signature
                             
                             eocdBuffer.getShort(); // Skip diskNumber
                             eocdBuffer.getShort(); // Skip centralDirDisk
                             int totalEntriesOnDisk = eocdBuffer.getShort() & 0xFFFF;
                             int totalEntries = eocdBuffer.getShort() & 0xFFFF;
                             long centralDirSize = eocdBuffer.getInt() & 0xFFFFFFFFL;
                             long centralDirOffset = eocdBuffer.getInt() & 0xFFFFFFFFL;
                             eocdBuffer.getShort(); // Skip commentLength
         ...
         private CompletableFuture<EndOfCentralDirectory> findZIP64EndOfCentralDirectory(
             ...
             long locatorStart = eocdOffset - ZIP64_EOCD_LOCATOR_SIZE;
             ...
             return fetchRange(client, basicAuth, locatorStart, locatorEnd)
                 .thenCompose(locatorData -> {
                     if (locatorData.length < ZIP64_EOCD_LOCATOR_SIZE) {
                     ...
                     return fetchRange(client, basicAuth, zip64EOCDOffset, zip64EOCDOffset + ZIP64_EOCD_RECORD_SIZE - 1)
                         .thenApply(zip64Data -> {
                             if (zip64Data.length < ZIP64_EOCD_RECORD_SIZE) {
                             ...
                             long totalEntriesOnDisk = zip64Buffer.getLong(); // Total entries on this disk
                             long totalEntries = zip64Buffer.getLong(); // Total entries in central directory
         ...
         private Map<String, ZipEntry> parseCentralDirectoryEntries(byte[] data, long centralDirOffset) {
             ...
             while (buffer.hasRemaining()) {
                 if (buffer.remaining() < MIN_CENTRAL_DIRECTORY_ENTRY_SIZE) break;
                 ...
                 int extraFieldStartPos = buffer.position();
                 ...
                 if (hasZip64Markers(compressedSize, uncompressedSize, localHeaderOffset)) {
                     ...
                     if (extraFieldLength > 0) {
                         ByteBuffer extraBuffer = buffer.slice();
                         extraBuffer.limit(extraFieldLength);
                 ...
                 // Advance main buffer past extra field and file comment
                 buffer.position(extraFieldStartPos + extraFieldLength + fileCommentLength);
             ...
             return client.sendAsync(request, HttpResponse.BodyHandlers.ofByteArray())
                 .thenApply(response -> {
                     logger.debug("HTTP Range response: {} (Server HTTP version: {}, Content-Length: {})", 
                                  response.statusCode(), response.version(), response.headers().firstValue("Content-Length").orElse("unknown"));
